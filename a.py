...
def overlay_clusters_on_image(img_array, label_image, alpha=0.4, point_size=0.1, background=None):
    """
    Overlay K-means cluster assignments on top of an image.

    Parameters
    ----------
    img_array : np.ndarray
        RGB image array (H, W, 3) used to derive dimensions and by default as background.
    label_image : np.ndarray
    ...
    background : Optional[Union[np.ndarray, tuple, matplotlib.figure.Figure]]
        A separate background to overlay on. Can be:
        - np.ndarray (H, W, 3)
        - (fig, ax) tuple from Matplotlib
        - matplotlib.figure.Figure (will create new axes to display)
    ...
    # Normalize background input to an RGB image we can display with imshow
    if background is None:
        bg_image = img_array
    elif isinstance(background, np.ndarray):
        bg_image = background
    elif isinstance(background, tuple) and len(background) == 2:
        # Convert provided (fig, ax) into an image so we can align pixel coordinates
        fig_bg, _ = background
        fig_bg.canvas.draw()
        bg_image = plt2rgbarr(fig_bg)
    elif hasattr(background, 'canvas'):
        background.canvas.draw()
        bg_image = plt2rgbarr(background)
    else:
        raise ValueError("Unsupported background type. Provide None, np.ndarray, (fig, ax), or Figure.")

    fig, ax = plt.subplots(figsize=(6, 6))
    ax.imshow(bg_image)

    ax.set_title(f"Clusters overlaid on image (k={k})")
    ax.axis("off")
